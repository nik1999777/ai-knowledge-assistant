...
export async function listDocuments(input: ListDocumentsInput = {}) {
  ...
  const result = tsQuery
    ? await pool.query<{ doc_id: string; title: string; ingestion_status: IngestionStatus }>(
        `
          SELECT doc_id, title, ingestion_status
          FROM documents
...
export async function updateIngestionStatus(docId: string, status: IngestionStatus) {
  await pool.query(
    "UPDATE documents SET ingestion_status = $1, updated_at = NOW() WHERE doc_id = $2",
    [status, docId],
  );
}
